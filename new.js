...
class XiaomiTTS {
  ...
  async serviceLoginAuth2() {
    ...
    try {
      ...
      await this.request
        .poet(
          `https://api2.mina.mi.com/admin/v2/device_list?master=0&requestId=app_ios_${getRandom(
            30
          )}`,
          {
            _nonce: "CZUcauDXahgBt7uM",
            data: '{"getVirtualModel": false, "getHuamiDevices": 0}',
            signature: "ZxLln6x9o0lImNi0hd4Jcp/wJLNeEPIHdS3O0y7CdbU=",
          },
          {
            headers: {
              Cookies: {
                userId: 1223109348,
                serviceToken:
                  "uTLQ7myPoSfus8UtYZhDiFwc+YXEkocJ1fPzQGmL0TKYqZTamx66wW8wpCiq0kGnbPHJxQp73u0GI3qd1eY8cp4BXbcIlaHYzSRvaW0fR7I29eDdCvQHea16T3Cp+FEQX/gv5r6LMX5azUk0ZFTDm3NGjaW4JXTkx1H13Wxc0Q4=",
                PassportDeviceId: "UAX8ZVPNXO3YIGHT",
              },
            },
          }
        )
  ...
  async loginMiai() {
    ...
    const serviceTokenSha1 = crypto
      .createHash("sha1")
      .update(serviceToken)
      .digest("base64");
    const url = `${
      this.serviceLoginAuth2Json.location
    }&clientSign=${encodeURIComponent(serviceTokenSha1)}`;
    console.log(url);
    try {
      const response = await this.request.get(url, {
        headers: {
  ...
  async getDeviceId() {
    const url =
      "https://api.mina.mi.com/admin/v2/device_list?master=1&requestId=someUniqueRequestId";
    try {
  ...
  async textToSpeech(text, deviceIdIndex = 0) {
    try {
      const url = `https://api.mina.mi.com/remote/ubus?deviceId=${
        this.deviceIds[deviceIdIndex].deviceID
      }&message=${encodeURIComponent(
        JSON.stringify({ text })
      )}&method=text_to_speech&path=mibrain&requestId=${this.generateRandomString(
        30
      )}`;
